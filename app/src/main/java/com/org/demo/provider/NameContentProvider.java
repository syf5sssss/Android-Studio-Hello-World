...
public class NameContentProvider extends ContentProvider {
    ...
    static {
        matcher.addURI("com.org.providers.MyProvider", "test", 1);
    }

    @Override
    public boolean onCreate() {
        dbOpenHelper = new DBOpenHelper(this.getContext(), "test.db", null, 1);
        Log.e("a","NameContentProvider onCreate");
        return true;
    }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        Log.e("a","NameContentProvider insert");
        switch (matcher.match(uri)) {
            //把数据库打开放到里面是想证明uri匹配完成
            case 1:
                SQLiteDatabase db = dbOpenHelper.getReadableDatabase();
                long rowId = db.insert("test", null, values);
                Log.e("a","NameContentProvider rowId:"+rowId);
                if (rowId > 0) {
                    //在前面已有的Uri后面追加ID
